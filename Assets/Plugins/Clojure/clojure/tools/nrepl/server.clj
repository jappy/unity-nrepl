...
(defn handle
  ...
  [handler transport]
  (when-let [msg (clojure.tools.nrepl.transport/recv transport)]
...
(defn stop-server
  ...
  [{:keys [open-transports ^Socket server-socket status] :as server}]             ;DM: ^ServerSocket
  (returning server
    #_(debug/prn-thread "Stopping server " (:port server)) ;DEBUG
